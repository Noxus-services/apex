import { Home, Dumbbell, BarChart2, Pill, User } from 'lucide-react'
        ...
        {tabs.map(({ id, label, icon: Icon }) => {
          ...
          return (
            ...
            >
              ...
              <Icon
                size={22}
                className={active ? 'text-accent-yellow' : 'text-[rgba(240,237,230,0.6)]'}
                strokeWidth={active ? 2.2 : 1.8}
              />
              <span
                className={`text-[10px] font-body font-medium leading-none ${
                  active ? 'text-accent-yellow' : 'text-[rgba(240,237,230,0.6)]'
                }`}
              >
                {label}
              </span>

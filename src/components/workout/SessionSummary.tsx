...
function StatCard({ label, value, sub }: { label: string; value: string | number; sub?: string }) {
  return (
    <div className="flex-1 flex flex-col items-center gap-1 bg-bg-elevated rounded-lg py-4 px-2">
      ...
      {sub && <span className="font-mono text-xs text-[rgba(240,237,230,0.4)]">{sub}</span>}
      <span className="text-xs uppercase tracking-widest text-[rgba(240,237,230,0.35)] font-body mt-1">
        {label}
      </span>
    </div>
  )
      ...
      <div className="flex-1 overflow-y-auto px-4 pt-5 flex flex-col gap-5">
              ...
              {session.prs.map((pr, i) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    <span className="font-mono text-xs text-[rgba(240,237,230,0.4)] line-through">
        ...
        <div className="card-elevated flex gap-4">
          <div className="flex-1 flex flex-col items-center gap-1">
            ...
            <span className="text-xs text-[rgba(240,237,230,0.4)] uppercase tracking-widest">Humeur</span>
          </div>
          <div className="w-px bg-border-subtle" />
          <div className="flex-1 flex flex-col items-center gap-1">
            <span className="text-2xl">{ENERGY_EMOJI[session.energy] ?? '⚡'}</span>
            <span className="text-xs text-[rgba(240,237,230,0.4)] uppercase tracking-widest">Énergie</span>
          </div>
        </div>

        {/* Exercise summary */}
        <div className="card">
          <p className="text-xs font-mono uppercase tracking-widest text-[rgba(240,237,230,0.4)] mb-3">
            EXERCICES
          </p>
          <div className="flex flex-col gap-2">
            {session.exercises.map((ex, i) => {
              ...
              return (
                <div key={i} className="flex items-center justify-between">
                  ...
                  <span className="font-mono text-xs text-[rgba(240,237,230,0.4)]">
        ...
        <div className="ai-card">
          ...
          {isStreaming && !aiMsg ? (
            <div className="flex items-center gap-2">
              <span className="text-sm text-[rgba(240,237,230,0.5)] animate-pulse">
                Analyse IA en cours...
              </span>
            </div>
          ) : aiMsg ? (
            ...
            <p className="text-sm text-[rgba(240,237,230,0.8)] leading-relaxed whitespace-pre-wrap">
              {streamingContent}
              <span className="inline-block w-1.5 h-4 bg-accent-blue/70 ml-0.5 animate-pulse" />
            </p>
          ) : (
            <p className="text-sm text-[rgba(240,237,230,0.4)]">
              L'analyse sera disponible après ta prochaine séance.
            </p>
          )}
        </div>
        ...
        {/* Notes */}
        <div className="flex flex-col gap-2">
          <label className="text-xs font-mono uppercase tracking-widest text-[rgba(240,237,230,0.4)]">
            NOTES
          </label>
          <textarea
            value={notes}
